...
*/
  ...
  cusparseCheck(cusparseCreateCsr(&Adescriptor,
								dimX,
								dimX,
								nzv,
								ARowPtr,
  ...
  cusparseCheck(cusparseCreateDnVec(&tempDescriptor,
									dimX,
									temp,
									CUDA_R_64F));
  cusparseCheck(cusparseCreateDnVec(&tmpDescriptor,
									dimX,
									tmp,
									CUDA_R_64F));
  ...
  for (int it = 0; it < nsteps; ++it) {
    ...
    cusparseCheck(cusparseSpMV(cusparseHandle,
							...
							buffer));
    time += cputimer_get();
    //cputimer_stop("Compute the SpMV");
	...
	printf("Compute the SMPV. \t\t\tElasped %.0f microseconds \n", time);
